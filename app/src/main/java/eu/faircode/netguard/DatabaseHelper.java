...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public Cursor getOrganizations() {
        ...
        try {
            ...
            // There is an index on time
            // There is no index on protocol/allowed for write performance
            String query = "SELECT organization,COUNT(*)";
            query += " FROM log";
            query += " WHERE organization is not null";
    ...
    public Cursor getCountries() {
        ...
        try {
            ...
            query += " FROM log";
            query += " WHERE country is not null";
    ...
    public long getLogCount() {
        ...
        try {
            SQLiteDatabase db = this.getReadableDatabase();
            long count = DatabaseUtils.queryNumEntries(db, "log");
            return count;
        } finally {
            lock.readLock().unlock();
        }
    ...
    public void insertLog(Packet packet, String dname, int connection, boolean interactive) {
        ...
        try {
            ...
            try {
                ...
                cv.put("organization", Util.getOrganization(packet.daddr));
                cv.put("country", Util.getCountry(packet.daddr));
